...
void App_NavMeshGraph::Update(float deltaTime)
{
	...
	if (sShowGraph)
	{
		auto nodes = m_pNavGraph->GetAllNodes();
		constexpr float circleRadius{1.f};

		auto connections = m_pNavGraph->GetAllConnections();

		for (auto conns : connections)
		{
			for (auto conn : conns)
			{
				auto fromNode = m_pNavGraph->GetNode(conn->GetFrom());
				auto toNode = m_pNavGraph->GetNode(conn->GetTo());

				DEBUGRENDERER2D->DrawSegment(fromNode->GetPosition(), toNode->GetPosition(), { 0,0,0,1 }, 0);
			}
