...
void App_AgarioGame::Start()
{
	...
	FSMStates::SeekFoodState* pSeekFoodState = new FSMStates::SeekFoodState();
	m_pStates.push_back(pSeekFoodState);

	//3. Create the transitions beetween those states
	FSMConditions::FoodNearByCondition* pFoodNearbyCondition = new FSMConditions::FoodNearByCondition();
	FSMConditions::FoodGoneCondition* pFoodGoneCondition = new FSMConditions::FoodGoneCondition();
	//FSMConditions::FoodSpawnNearByCondition* pFoodSpawnNearbyCondition = new FSMConditions::FoodSpawnNearByCondition();

	m_pConditions.push_back(pFoodNearbyCondition);
	m_pConditions.push_back(pFoodGoneCondition);
	//m_pConditions.push_back(pFoodSpawnNearbyCondition);
	...
	pFSM->AddTransition(pWanderState,pSeekFoodState, pFoodNearbyCondition);
	pFSM->AddTransition(pSeekFoodState, pWanderState, pFoodGoneCondition);
	//pFSM->AddTransition(pSeekFoodState, pSeekFoodState, pFoodSpawnNearbyCondition);
