...
void App_AgarioGame_BT::Start()
{
	...
	for (int i = 0; i < m_AmountOfAgents; i++)
	{
		...
		//1. Create Blackboard
		auto pBlackBoard = CreateBlackboard(newAgent);
				
		//2. Create BehaviorTree
		auto pTree = new BehaviorTree(pBlackBoard, new BehaviorAction(BT_Actions::ChangeToWander));
		//3. Set the BehaviorTree active on the agent 
		newAgent->SetDecisionMaking(pTree);
